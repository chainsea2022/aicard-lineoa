...
export const ExpandedCard: React.FC<ExpandedCardProps> = ({
  ...
  const handleAddNewTag = () => {
    console.log('handleAddNewTag called', {
  ...
  const handleRemoveSelectedTag = (tag: string) => {
    console.log('handleRemoveSelectedTag called', {
      tag,
      customerId: customer.id
    });
  ...
  return <div className="space-y-4">
      ...
      {!customer.isRegisteredUser && !customer.lineId && <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 space-y-3 bg-white">
          ...
          <TooltipProvider>
            <div className="flex justify-center space-x-4">
              {/* SMS */}
              {customer.phone && <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="relative">
                      {invitationStatus.sms && <div className="absolute -top-2 -right-2 w-4 h-4 bg-green-600 rounded-full flex items-center justify-center">
                          <CheckCircle className="w-3 h-3 text-white" />
                        </div>}
                      <Button variant="ghost" size="sm" className="w-12 h-12 p-0 flex items-center justify-center hover:bg-white rounded-full" onClick={() => invitationStatus.sms ? setShowInvitationHistory(true) : handleInvitationAction('sms')}>
              ...
              {customer.email && <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="relative">
                      {invitationStatus.email && <div className="absolute -top-2 -right-2 w-4 h-4 bg-green-600 rounded-full flex items-center justify-center">
                          <CheckCircle className="w-3 h-3 text-white" />
                        </div>}
                      <Button variant="ghost" size="sm" className="w-12 h-12 p-0 flex items-center justify-center hover:bg-white rounded-full" onClick={() => invitationStatus.email ? setShowInvitationHistory(true) : handleInvitationAction('email')}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="relative">
                    {invitationStatus.line && <div className="absolute -top-2 -right-2 w-4 h-4 bg-green-600 rounded-full flex items-center justify-center">
                        <CheckCircle className="w-3 h-3 text-white" />
                      </div>}
                    <Button variant="ghost" size="sm" className="w-12 h-12 p-0 flex items-center justify-center hover:bg-white rounded-full" onClick={() => invitationStatus.line ? setShowInvitationHistory(true) : handleInvitationAction('line')}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="relative">
                    {invitationStatus.messenger && <div className="absolute -top-2 -right-2 w-4 h-4 bg-green-600 rounded-full flex items-center justify-center">
                        <CheckCircle className="w-3 h-3 text-white" />
                      </div>}
                    <Button variant="ghost" size="sm" className="w-12 h-12 p-0 flex items-center justify-center hover:bg-white rounded-full" onClick={() => invitationStatus.messenger ? setShowInvitationHistory(true) : handleInvitationAction('messenger')}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="relative">
                    {invitationStatus.instagram && <div className="absolute -top-2 -right-2 w-4 h-4 bg-green-600 rounded-full flex items-center justify-center">
                        <CheckCircle className="w-3 h-3 text-white" />
                      </div>}
                    <Button variant="ghost" size="sm" className="w-12 h-12 p-0 flex items-center justify-center hover:bg-white rounded-full" onClick={() => invitationStatus.instagram ? setShowInvitationHistory(true) : handleInvitationAction('instagram')}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="relative">
                    {invitationStatus.copy && <div className="absolute -top-2 -right-2 w-4 h-4 bg-green-600 rounded-full flex items-center justify-center">
                        <CheckCircle className="w-3 h-3 text-white" />
                      </div>}
                    <Button variant="ghost" size="sm" className="w-12 h-12 p-0 flex items-center justify-center hover:bg-white rounded-full" onClick={() => invitationStatus.copy ? setShowInvitationHistory(true) : handleInvitationAction('copy')}>
        ...
        <DialogContent className="max-w-[300px] mx-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Button variant="outline" className="w-full" onClick={() => setShowInvitationDialog(false)}>
                取消
              </Button>
            </div>
          </div>
      ...
      <Dialog open={showInvitationHistory} onOpenChange={setShowInvitationHistory}>
        <DialogContent className="max-w-[300px] mx-auto">
          ...
          <div className="space-y-3">
            {Object.entries(invitationHistory).map(([platform, time]) => <div key={platform} className="flex items-center justify-between p-2 bg-gray-50 rounded">
            ...
            {Object.keys(invitationHistory).length === 0 && <p className="text-center text-gray-500 text-sm">尚無邀請紀錄</p>}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setShowAddTag(true)} variant="ghost" size="sm" className="text-xs text-blue-600 hover:text-blue-700">
        ...
        {/* 已選擇的標籤 - 第一行 */}
        {localCustomer.tags && localCustomer.tags.length > 0 && <div className="flex gap-2 overflow-x-auto pb-2">
            {localCustomer.tags.map((tag, index) => {
          const isSelected = selectedTags.has(tag);
          return <div key={`selected-${index}`} className="relative flex-shrink-0 group">
                  <Badge variant="secondary" className={`text-xs cursor-pointer px-3 py-1 rounded-full transition-colors ${isSelected ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'}`} onClick={() => handleTagClick(tag)}>
                    {tag}
                  </Badge>
                  {isSelected && <Button onClick={e => {
              e.stopPropagation();
              handleRemoveSelectedTag(tag);
            }} variant="ghost" size="sm" className="absolute -top-1 -right-1 w-4 h-4 p-0 bg-red-500 hover:bg-red-600 text-white rounded-full transition-opacity">
                      <X className="w-2 h-2" />
                    </Button>}
                </div>;
        })}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {generateAutoTags().filter(tag => !localCustomer.tags?.includes(tag)).map((tag, index) => <div key={`ai-${index}`} className="relative flex-shrink-0 group">
              <Badge variant="outline" className="text-xs bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100 cursor-pointer px-3 py-1 rounded-full transition-colors" onClick={() => handleAITagClick(tag)}>
                {tag}
              </Badge>
            </div>)}
        </div>
        ...
        {/* 新增標籤輸入框 */}
        {showAddTag && <div className="flex gap-2 items-center">
            <Input value={newTagInput} onChange={e => setNewTagInput(e.target.value)} placeholder="輸入自訂標籤" className="text-sm flex-1" onKeyPress={e => {
          if (e.key === 'Enter') {
            handleAddNewTag();
          }
        }} autoFocus />
        ...
        {allTags.length === 0 && !showAddTag && <p className="text-xs text-gray-500 text-center py-4">
