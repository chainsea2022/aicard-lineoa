...
export const SmartRecommendation: React.FC<SmartRecommendationProps> = ({
  ...
  const showUpgradePrompt = addedCount >= 10;
          ...
          {showUpgradePrompt && (
            <div className="mb-4 p-3 bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-lg">
              <div className="text-center">
                <h3 className="text-sm font-semibold text-purple-800 mb-1">升級至 Aile Pro</h3>
                <p className="text-xs text-purple-600 mb-2">您已加入 10 張名片，升級解鎖商務全功能！</p>
                <Button size="sm" className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white text-xs">
                  立即升級
                </Button>
              </div>
            </div>
          ...
          <Carousel className="w-full">
            <CarouselContent className="-ml-2 md:-ml-4">
              {recommendations.map((contact) => (
                <CarouselItem key={contact.id} className="pl-2 md:pl-4 basis-[280px]">
